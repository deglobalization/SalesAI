...
function convertToStaticPaths() {
    ...
    const apiMappings = {
        ...
        // localhost:8101 포트 매핑
        'http://localhost:8101/api/products': './product_groups.json',
        'http://localhost:8101/api/managers': './manager_list.json',
        'http://localhost:8101/api/recommendations': './recommendations_data.json',
        'http://localhost:8101/api/manager-recommendations': './manager_recommendations_data.json',
        'http://localhost:8101/api/manager-focus': './manager_focus_regions.json',
        'http://localhost:8101/api/summary': './total_managers_summary.json',
        'http://localhost:8101/api/health': './health.json',
        'http://localhost:8101/api/recommend': './recommendations_data.json',
    ...
    try {
        ...
        if (isGitHubPages()) {
            ...
            if (!staticPath && typeof url === 'string') {
                if (url.includes('localhost:8080/api/products') || url.includes('localhost:8101/api/products')) {
                    staticPath = './product_groups.json';
                    console.log(`✅ 3단계 패턴 매칭: products API → "${staticPath}"`);
                } else if (url.includes('localhost:8080/api/recommend') || url.includes('localhost:8101/api/recommend')) {
                    staticPath = './recommendations_data.json';
                    console.log(`✅ 3단계 패턴 매칭: recommend API → "${staticPath}"`);
                } else if (url.includes('localhost:8080/api/managers') || url.includes('localhost:8101/api/managers')) {
                    staticPath = './manager_list.json';
                    console.log(`✅ 3단계 패턴 매칭: managers API → "${staticPath}"`);
                } else if (url.includes('localhost:8080/api/') || url.includes('localhost:8101/api/')) {
                    staticPath = './health.json';  // 기본 fallback
            ...
            if (options.method === 'POST') {
                ...
                // 추천 요청인 경우 실제 recommendations_data.json에서 데이터 로드
                if (url.includes('recommend')) {
                    try {
                        // POST 요청의 body에서 product_group과 manager 추출
                        let productGroup = null;
                        let requestedManager = '전체';
                        if (options.body) {
                            const requestData = JSON.parse(options.body);
                            productGroup = requestData.product_group;
                            requestedManager = requestData.manager || '전체';
                        }
                        
                        console.log(`🔍 요청된 품목군: "${productGroup}", 담당자: "${requestedManager}"`);
                        
                        // 담당자별 추천 데이터 사용 결정
                        let dataFile = './recommendations_data.json';
                        if (requestedManager !== '전체') {
                            dataFile = './manager_recommendations_data.json';
                            console.log(`👤 담당자별 데이터 사용: ${dataFile}`);
                        }
                        
                        // 추천 데이터 로드
                        const recommendationsResponse = await originalFetch(dataFile);
                        if (!recommendationsResponse.ok) {
                            throw new Error('추천 데이터 파일을 찾을 수 없습니다.');
                        }
                        ...
                        // 담당자별 데이터 구조 처리
                        if (requestedManager !== '전체' && recommendationsData.managers) {
                            // 특정 담당자의 데이터만 추출
                            const managerData = recommendationsData.managers[requestedManager];
                            if (managerData && managerData.recommendations) {
                                allRecommendations = managerData.recommendations;
                                console.log(`✅ "${requestedManager}" 담당자 데이터 로드 완료`);
                            } else {
                                console.log(`❌ "${requestedManager}" 담당자 데이터 없음, 전체 데이터 사용`);
                        ...
                        if (productGroup && allRecommendations[productGroup]) {
                            ...
                            console.log(`✅ "${productGroup}" 품목군 데이터 발견: ${productRecommendations.length}개 추천 (담당자: ${requestedManager})`);
                        } else {
                            console.log(`❌ "${productGroup}" 품목군 데이터 없음. 사용 가능한 품목군:`, Object.keys(allRecommendations).slice(0, 5));
                        }
                        
                        // 최대 20개까지 제한하고 성공확률 기준으로 정렬
                        const limitedRecommendations = productRecommendations
                        ...
                        return {
                            ...
                            json: async () => ({
                                ...
                                recommendations: limitedRecommendations.map(rec => ({
                                    ...
                                    analysis: `유사도 점수: ${rec.유사도점수 || rec.similarity_score || 'N/A'}, 성공 확률: ${rec.성공확률 || rec.success_probability || 'N/A'}%`,
                                    strategies: [
                                        `추천 이유: ${rec.추천이유 || rec.reason || '데이터 기반 추천'}`,
                                        `예상 매출: ${rec.예상매출 || rec.expected_sales || 'N/A'}원`
                                    ],
                                    confidence: (rec.성공확률 || rec.success_probability || 75) / 100,
                                    productName: productGroup,
                                    rawData: rec // 원본 데이터도 포함
                                })),
                                message: `GitHub Pages에서 "${productGroup}" 품목군에 대한 ${limitedRecommendations.length}개 추천을 제공합니다.`,
                                source: 'static_data',
                        ...
                        console.error('🚨 실제 추천 데이터 로드 실패:', error);
